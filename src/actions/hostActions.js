...
export const addFood = (id, food) => (dispatch) => {
  ///foods/potluck/{potluckid}/food/
  dispatch({ action: ADD_FOOD_START });
  axios
    .post(`https://lre-notapotluck.herokuapp.com/foods/potluck/${id}/food`, food)
...
export const addGuest = (id, guest) => (dispatch) => {
  ...
  axios
  .post(`https://lre-notapotluck.herokuapp.com/guests/potluck/${id}`, guest)
  .then(res => dispatch({action: ADD_FOOD_SUCCESS, payload: res.data}))
